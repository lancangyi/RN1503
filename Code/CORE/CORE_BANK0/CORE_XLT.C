...
const FUNCT_PTR_V_BS code kcp_vector_table[] =
{
    ...
    control_effect,     // index 05h DO_CONTROL_EFFECT Shift, Ctrl, Alt, Numlock CapsLock
    control_effect_e0,	// index 06h DO_CONTROL_EFFECT_E0 Ctrl, Alt 
    customer_function	// index 07h DO_COSTOMER_FUNCTION costomer function key
};
...
//----------------------------------------------------------------------------
static void customer_function(BYTE code_byte, BYTE event)
{
...
static void control_effect(BYTE state, BYTE event)
{
    if ((event == MAKE_EVENT)||(event == BREAK_EVENT))
    {
    	...
    	else if(state & CAP_LOCK)//CapsLock 
    	{
            //add code to send scan code to the host
		    simple_code(0x58, event);
		}
    }
